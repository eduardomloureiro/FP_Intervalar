...
% Incerteza associada a geração e as cargas

E = 0.05; %valor da incerteza associada
%Pg_inf = DADOS_BARRA(:,4)*(1-E);
%Pg_sup = DADOS_BARRA(:,4)*(1+E);
Pl_inf = DADOS_BARRA(:,6)*(1-E);
Pl_sup = DADOS_BARRA(:,6)*(1+E);
Ql_inf = DADOS_BARRA(:,7)*(1-E);
Ql_sup = DADOS_BARRA(:,7)*(1+E);

% Atualização da geração e das cargas na matriz DADOS_BARRA

%DADOS_BARRA(:,4)=Pg_inf+rand*(Pg_sup-Pg_inf);
DADOS_BARRA(:,6)=Pl_inf+rand*(Pl_sup-Pl_inf);
DADOS_BARRA(:,7)=Ql_inf+rand*(Ql_sup-Ql_inf);
...
%salva V e theta intervalar
armaz_V.V_int=[min(([armaz_V.V])')' max(([armaz_V.V])')'];
armaz_V.Ang_int=[min(([armaz_V.Ang])')' max(([armaz_V.Ang])')'];
save('armaz_V')

%Tensão Intervalar Probabilística
armaz_V.V_int

%Ângulo Intervalar Probabilístico - rad
armaz_V.Ang_int